...
class CommunicationMessageRecipient(
    ...
    def clean(self) -> None:
        ...
        identified_recipient_count = sum(
        ...
        is_direct_email = (
            ...
            and bool(self.destination_email)
        )

        if (
            identified_recipient_count == 0
            and not is_direct_email
        ):

            errors["user"] = (

                "Un destinataire ou une adresse email "
                "doit être renseigné."

            )


        if identified_recipient_count > 1:
